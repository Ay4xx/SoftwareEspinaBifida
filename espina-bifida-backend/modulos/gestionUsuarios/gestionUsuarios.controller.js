...
function getFotoBuffer(req) {
  return req.file ? req.file.buffer : undefined;
}
...
export async function crear(req, res) {
  try {
    const nuevo = await crearUsuario({ ...req.body, foto: getFotoBuffer(req) });
...
export async function actualizar(req, res) {
  try {
    const actualizado = await actualizarUsuario(Number(req.params.id), { ...req.body, foto: getFotoBuffer(req) });
