...
export async function registrarPaciente(req, res) {
  try {
    const { nombre, apellido, genero, fechaNacimiento, curp } = req.body;
    ...
    const resultado = await crearPacientePaso1({ nombre, apellido, genero, fechaNacimiento, curp });
...
export async function contactoPaciente(req, res) {
  try {
    ...
    const { direccion, ciudad, estado, codigoPostal,
            ...
            telefonoCasa, telefonoCelular, correo } = req.body;
