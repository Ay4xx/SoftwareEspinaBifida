...
export async function obtenerHistorialPorPaciente(pacienteId) {
  ...
  try {
    ...
    const result = await conn.execute(
      `SELECT *
       FROM (
          SELECT v.EVENTO_ID, v.MONTO_RECIBIDO, v.PACIENTE_ID, v.FECHA_EVENTO,
                 'servicio' AS tipo, s.NOMBRE_SERVICIO AS nombre,
                 (s.COSTO * 1) AS precio, 1 AS cantidad
          ...
          SELECT v.EVENTO_ID, v.MONTO_RECIBIDO, v.PACIENTE_ID, v.FECHA_EVENTO,
                 'medicamento' AS tipo, i.DESCRIPCION AS nombre,
                 (i.PRECIO * m.CANTIDAD_RESTA) AS precio, m.CANTIDAD_RESTA AS cantidad
          ...
          SELECT v.EVENTO_ID, v.MONTO_RECIBIDO, v.PACIENTE_ID, v.FECHA_EVENTO,
                 'equipo' AS tipo, iep.DESCRIPCION AS nombre,
                 (iep.PRECIO * ep.CANTIDAD_RESTA) AS precio, ep.CANTIDAD_RESTA AS cantidad
