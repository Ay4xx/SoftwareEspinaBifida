...
export async function obtenerHistorialPorPaciente(pacienteId) {
  ...
  try {
    ...
    const result = await conn.execute(
      `SELECT *
       FROM (
          SELECT v.PACIENTE_ID, v.FECHA_EVENTO, 'servicio' AS tipo, s.NOMBRE_SERVICIO AS nombre, s.COSTO AS precio
          ...
          SELECT v.PACIENTE_ID, v.FECHA_EVENTO, 'medicamento' AS tipo, i.DESCRIPCION AS nombre, i.PRECIO AS precio
          ...
          SELECT v.PACIENTE_ID, v.FECHA_EVENTO, 'equipo' AS tipo, iep.DESCRIPCION AS nombre, iep.PRECIO AS precio
