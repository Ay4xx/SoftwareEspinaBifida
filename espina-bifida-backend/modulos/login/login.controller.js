...
function generarToken(data) {
  return `token-${data.id}-${data.tipoUsuario}-${Date.now()}`;
}
...
export async function loginPaciente(req, res) {
  try {
    ...
    if (!username || !password) {
      return res.status(400).json({ ok: false, message: "Username y contraseña son obligatorios" });
    }
    ...
    res.json({ ok: true, token: generarToken(data), data });
  } catch (error) {
