...
const MIME = { excel: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", pdf: "application/pdf", csv: "text/csv" };
...
function descargarArchivo(blob, tipoArchivo) {
  ...
  a.href     = url;
  a.download = `reporte_mensual.${EXT[tipoArchivo]}`;
...
export default function ReporteMensualModal({ open, onClose }) {
  ...
  const toggleSection  = (key)   => setFormData((p) => ({ ...p, [key]: !p[key] }));
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      setLoading(false);
      if (success) setTimeout(onClose, 0);
    }
  };
      ...
      <div className="rm-modal">
        ...
        <div className="rm-section-label">
          ...
          <div className="rm-quick-btns">
            <button className="rm-link-btn" onClick={() => setAllSections(true)}>Todas</button>
            <span className="rm-link-sep">·</span>
            <button className="rm-link-btn" onClick={() => setAllSections(false)}>Ninguna</button>
          </div>
        </div>

        <div className="rm-sections-grid">
          {SECCIONES.map(({ key, label, icon: Icon, desc }) => (
            <label key={key} className={`rm-section-card ${formData[key] ? "checked" : ""}`}>
              <input type="checkbox" checked={formData[key]} onChange={() => toggleSection(key)} className="rm-hidden-check" />
              <div className="rm-card-icon"><Icon size={16} strokeWidth={2} /></div>
