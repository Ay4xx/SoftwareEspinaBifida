...
import {
  ...
  DollarSign, Bell, Activity, Package, TrendingUp,
...
export default function EstadisticasPage() {
  ...
  /* ── derived series merged for combo charts ── */
  const citasSerie = mergeSeries(
    series.citasMes,          "total_citas",
    series.citasAtendidasMes, "atendidas",
    series.citasCanceladasMes,"canceladas"
  );

  const ingresosSerie = mergeSeries(
    series.ingresosMes,   "ingresos",
    series.descuentosMes, "descuentos"
  );

  const serviciosSerie = mergeSeries(
    series.visitasMes,               "visitas",
    series.serviciosMes,             "servicios",
    series.medicinasUtilizadasMes,   "medicinas"
  );

  const equipoSerie = series.equiposEnUsoMes.map((r) => ({ mes: r.mes, en_uso: r.total }));

  const pacientesSerie = series.pacientesNuevosMes.map((r) => ({ mes: r.mes, nuevos: r.total }));

  const notifSerie = series.notificacionesMes.map((r) => ({ mes: r.mes, notif: r.total }));
  ...
  const tablaIngresos = ingresosSerie.map((r) => ({
    mes: r.mes,
    ingresos: r.ingresos ?? 0,
    descuentos: r.descuentos ?? 0,
    neto: (r.ingresos ?? 0) - (r.descuentos ?? 0),
  }));

  const tablaServicios = serviciosSerie.map((r) => ({
    mes: r.mes,
    visitas: r.visitas ?? 0,
    servicios: r.servicios ?? 0,
    medicinas: r.medicinas ?? 0,
  }));

  /* ── donut data ── */
  const donutMembresias = [
    { name: "Activas", value: membresias.activas },
    { name: "Inactivas", value: membresias.inactivas },
    { name: "Vencidas", value: membresias.vencidas },
  ];
  const donutEquipo = [
    { name: "En uso", value: equipo.en_uso },
    { name: "Regresados", value: equipo.regresados },
  ];
  const donutCitas = [
    { name: "Atendidas", value: citas.atendidas },
    { name: "Canceladas", value: citas.canceladas },
    { name: "Pendientes", value: citas.pendientes },
  ];
    ...
    <div className="estadisticas-page">
      ...
      <nav className="dash-nav">
        {navItems.map((n) => (
          <button
            key={n.key}
            className={`dash-nav-btn ${activeSection === n.key ? "active" : ""}`}
          ...
          <div className="charts-row-2">
            <ChartCard title="Visitas por mes">
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={series.visitasMes.map(r => ({ mes: r.mes, total: r.total }))}>
                  <defs>
      ...
      {activeSection === "pacientes" && (
        <section>
          ...
          <DataTable
            ...
            rows={pacientesSerie.map((r) => ({ mes: r.mes, nuevos: r.nuevos }))}
            columns={[
              { key: "mes",    label: "Mes" },
              { key: "nuevos", label: "Nuevos", numeric: true, format: fmt },
            ]}
            filterKey="mes"
          ...
          <div className="charts-row-2">
            <ChartCard title="Citas por mes">
              ...
              <ChartLegend items={[
                ...
                { color: COLORS.teal, label: "Atendidas", dash: true },
                { color: COLORS.red,  label: "Canceladas", dash: true },
              ]} />
            </ChartCard>
        ...
        <section>
          ...
          <div className="charts-row-2">
            ...
            <ChartCard title="Visitas y servicios por mes">
              ...
              <ChartLegend items={[
                { color: COLORS.blue,   label: "Visitas" },
                { color: COLORS.purple, label: "Servicios", dash: true },
                { color: COLORS.amber,  label: "Medicinas", dash: true },
              ]} />
            </ChartCard>
          </div>
          ...
          <DataTable
            ...
            columns={[
              { key: "mes",       label: "Mes" },
              { key: "visitas",   label: "Visitas",   numeric: true, format: fmt },
              { key: "servicios", label: "Servicios", numeric: true, format: fmt },
              { key: "medicinas", label: "Medicinas", numeric: true, format: fmt },
            ]}
            filterKey="mes"
            filterLabel="Filtrar por mes…"
          />
        ...
        <section>
          ...
          <div className="charts-row-2">
            <ChartCard title="Medicinas utilizadas por mes">
              <ResponsiveContainer width="100%" height={230}>
                <BarChart data={series.medicinasUtilizadasMes.map(r => ({ mes: r.mes, utilizadas: r.total }))}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(0,0,0,.06)" />
                  <XAxis dataKey="mes" tick={{ fontSize: 11 }} tickLine={false} />
                  <YAxis tick={{ fontSize: 11 }} tickLine={false} axisLine={false} allowDecimals={false} />
                  <Tooltip content={<CustomTooltip />} />
          ...
          <div className="charts-row-2">
            ...
            <ChartCard title="Actualizaciones de inventario por mes">
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={series.actualizacionesMes.map(r => ({ mes: r.mes, actualizaciones: r.total }))}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(0,0,0,.06)" />
                  <XAxis dataKey="mes" tick={{ fontSize: 11 }} tickLine={false} />
                  <YAxis tick={{ fontSize: 11 }} tickLine={false} axisLine={false} allowDecimals={false} />
                  <Tooltip content={<CustomTooltip />} />
