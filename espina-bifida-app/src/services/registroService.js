...
export async function actualizarPaso5(pacienteId, foto) {
  ...
  if (!response.ok) {
    throw new Error(data.message || "Error al guardar la fotografía.");
  }
...
export async function actualizarPaso4(pacienteId, formData) {
  ...
  if (!response.ok) {
    throw new Error(data.message || "Error al guardar historial del tutor.");
  }
...
export async function actualizarPaso3(pacienteId, formData) {
  ...
  if (!response.ok) {
    throw new Error(data.message || "Error al guardar historial médico.");
  }
